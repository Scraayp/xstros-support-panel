...
<div x-data="{ showNotifications: false }" class="relative">
    ...
    <div x-cloak x-show="showNotifications" @click.away="showNotifications = false" class="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden z-50">
        ...
        <div class="max-h-60 overflow-y-auto">
            @forelse(auth()->user()->unreadNotifications as $notification)
                <div class="relative px-4 py-3 flex items-center space-x-3 hover:bg-gray-100 dark:hover:bg-gray-700">
                    <!-- User Avatar -->
                    <div class="shrink-0">
                        <img class="rounded-full w-11 h-11" src="/pfp-placeholder.png" alt="{{ $notification->data['reply_creator'] }}">
                    </div>

                    <!-- Notification Content -->
                    <div class="w-full">
                        <a href="{{ route('ticket.view', $notification->data['ticket_id']) }}" class="block text-gray-700 dark:text-gray-300 text-sm">
                            <strong class="text-lg">{{ $notification->data['reply_creator'] }} |
                                <span class="@if($notification->data['reply_creator_role'] === 'Admin') text-blue-500
                                             @elseif($notification->data['reply_creator_role'] === 'Staff') text-green-500
                                             @else text-gray-400
                                             @endif">
                                    {{ $notification->data['reply_creator_role'] }}
                                </span>
                            </strong>
                            Replied to your ticket (#{{ $notification->data['ticket_id'] }})
                        </a>
